...
function ConnectionIndicator({ status, lastUpdate }: { status: ConnectionStatus; lastUpdate: Date | null }) {
  const [dataAge, setDataAge] = useState(0)
  
  useEffect(() => {
    const interval = setInterval(() => {
      const updateTime = lastUpdate ? lastUpdate.getTime() : Date.now()
      setDataAge(Math.floor((Date.now() - updateTime) / 1000))
    }, 1000)
  ...
  const getStatusConfig = (s: ConnectionStatus) => {
    switch (s) {
      ...
      case 'error':
        return { icon: WifiOff, color: 'text-red-500', bg: 'bg-red-500', label: 'Error' }
      case 'disconnected':
      default:
