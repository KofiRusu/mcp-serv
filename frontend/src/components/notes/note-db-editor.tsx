...
export function NoteDBEditor({ note, onUpdate, onDelete, onSendToChat }: NoteDBEditorProps) {
  ...
  const handleCreateTasks = async () => {
    ...
    try {
      ...
      const newTasks = await createTasksFromNote(note.id)
      setTasks(newTasks)
      
      if (newTasks.length > 0) {
        // Show success feedback
        alert(`Created ${newTasks.length} task(s) from action items!`)
      } else {
                ...
                <div className="mt-4 pt-4 border-t border-[var(--accent-primary)]/20">
                  ...
                  <ul className="space-y-2">
                    {actionItems.map((item, index) => {
                      const matchingTask = tasks.find(t => t.content === item)
                      return (
                        <li
                          key={index}
                        ...
                        >
                          ...
                          {matchingTask && (
                            <span className="text-xs text-[var(--text-muted)] ml-auto">
                              {matchingTask.completed ? 'Completed' : 'Pending'}
                            </span>
                          )}
                        </li>
                      )
