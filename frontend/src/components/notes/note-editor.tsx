...
export function NoteEditor({ note, onUpdate, onDelete }: NoteEditorProps) {
  ...
  const handleExtractActions = async () => {
    try {
      ...
      const actions = await extractActions(note.id)
      if (actions.length > 0) {
        // Update note with extracted action items
        const updated = { ...note, action_items: [...(note.action_items || []), ...actions] }
        onUpdate(updated)
        setShowActions(true)
      }
    } catch (error) {
