 ...
 * Returns the full message object if JSON, or wraps string in object
 */
export function consumePendingChatMessage(): PendingChatMessage | null {
  ...
  // Try to parse as JSON, fall back to wrapping raw string
  if (message) {
    try {
      return JSON.parse(message) as PendingChatMessage
    } catch {
 ...
 */
export function formatSearchResultForChat(
  type: string,
  title: string,
  snippet: string
): string {
