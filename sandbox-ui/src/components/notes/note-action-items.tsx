...
export function NoteActionItems({ note, onUpdate }: NoteActionItemsProps) {
  ...
  const handleToggleComplete = async (action: ActionItem) => {
    try {
      ...
      if (action.status === "completed") {
        // Reopen
        const updated = await updateAction(note.id, action.id, { status: "pending" })
        onUpdate({
          ...note,
        ...
        // Complete
        const result = await completeAction(note.id, action.id)
        onUpdate({
          ...note,
          action_items: note.action_items.map((a) =>
            a.id === action.id ? result.action : a
          ),
        })
      }
    } catch (error) {
  ...
  const handleDelete = async (actionId: string) => {
    try {
      ...
      await deleteAction(note.id, actionId)
      onUpdate({
        ...note,
  ...
  const handleConvertToTask = async (action: ActionItem) => {
    try {
      setLoadingId(action.id)
      const result = await convertToTask(note.id, action.id)
      if (result.success) {
        onUpdate({
          ...note,
          action_items: note.action_items.map((a) =>
  ...
  const handleSetPriority = async (action: ActionItem, priority: ActionPriority) => {
    try {
      setLoadingId(action.id)
      const updated = await updateAction(note.id, action.id, { priority })
      onUpdate({
        ...note,
        action_items: note.action_items.map((a) =>
          a.id === action.id ? updated : a
  ...
  const pendingActions = note.action_items.filter((a) => a.status !== "completed")
  const completedActions = note.action_items.filter((a) => a.status === "completed")
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          {action.linked_task_id && (
            <span className="text-xs px-1.5 py-0.5 rounded bg-[var(--success)]/10 text-[var(--success)]">
              Linked
            </span>
          )}
        </div>
